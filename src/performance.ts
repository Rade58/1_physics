...
// somtimes when you create a lot of objects perfomance can drop

// --------------------------- BROADPHASE

// CANNON TESTS EVERY COLLISION BETWEEN OBJECTS, obviouslly this can impact performance
// a lot of work for cpu since cannon uses cpu

// doesn't matter that objects are too far from eachother, they are being tested

// we can use different values for broadphase to lower impact on performance

// - NaiveBroadphase - default, tests every body against every other body
// - GridBroadphase - tests only bodies in same grid box or neighbors grid bodies (everything will be divided to grids)
//                                    (can be problematic if you have fast body tat moves accros multiple grids)
// - SAPBroadphase (Sweep And Prune) - tests Bodies on arbitrary axes during multiple steps (author of workshop doesn't understand it)
// he just know that it is better than previous ones in many situations
// but if object is too fast it can happen that it won't colide with other objects

// we will set it by doing this for example           world.broadphase = new CANNON.SAPBroadphase(world)

// ---------------------------  SLEEP
// WE HAVE A PROBLEM WHERE CANNON TEST OBJECTS THAT AREN'T MOVING ANYMORE
// when body gets slow it can fall asleep and won't be tested unless aa sufficient force is applied

// for example when force needs to be applied to some body that is stilll, you can wake it up to start testing it again
// and if it' stop you put it to sleep again; and this will drastically improve performance

// We set this
// world.allowSleep = true

// you can also change       sleepSpeedLimit or sleepTimeLimet, but we won't change these

// just for you to know that we did huge optimizations just with few lines of code:
